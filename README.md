...
An [AWS IoT policy](https://docs.aws.amazon.com/iot/latest/developerguide/iot-policy-actions.html) configures what the device/thing will be allowed to do, e.g., iot:Connect (permission to connect to AWS IoT Core), iot:Receive (permission to receive a message from AWS IoT Core), iot:Publish  (permission to publish to a topic) and iot:Subscribe (permission to subscribe to a topic)
